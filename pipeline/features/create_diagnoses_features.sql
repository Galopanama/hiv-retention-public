...
create table features_cs.diag_info as
	...
	left join lookup_ucm.icd10_info info10 USING (icd10_code)
        where event_type = 'visit' and visit_date IS NOT NULL);
...
create temp table temp_diag2 as
       (select diagnosis
        from temp_diag
        group by diagnosis
        order by count(diagnosis) desc limit 10
       );
create temp table temp_diag3 as
       (select ccs_cat
        from temp_diag
        group by ccs_cat
        order by count(ccs_cat) desc limit 10
       );
...
create table features_cs.diagnoses as
       ...
       full outer join features_cs.hospital_stays using (entity_id, date_col)
       );
